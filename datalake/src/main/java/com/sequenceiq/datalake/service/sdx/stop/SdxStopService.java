...
public class SdxStopService {
    ...
    public void triggerStop(SdxCluster cluster) {
    ...
    private AttemptResult<StackV4Response> getStackResponseAttemptResult(SdxCluster sdxCluster) throws JsonProcessingException {
        ...
        if (stackAndClusterStopped(stackV4Response, cluster)) {
            ...
            } else if (Status.STOP_FAILED.equals(stackV4Response.getCluster().getStatus())) {
                LOGGER.info("Cluster stop failed for Cluster {} status {}, reason", stackV4Response.getCluster().getName(),
                        stackV4Response.getCluster().getStatus(), stackV4Response.getStatusReason());
                return sdxStopFailed(sdxCluster, stackV4Response.getCluster().getStatusReason());
            } else {
