...
public class ServiceEndpointCollectorTest {
    ...
    public void setup() {
        ...
        ReflectionTestUtils.setField(underTest, "knoxPort", "443");
    }
    ...
    public void testGetCmServerUrlWithAmbariPresentInTopologiesWithIndividualGateway() {
        ...
        String result = underTest.getManagerServerUrl(cluster, CLOUDERA_MANAGER_IP);
        assertEquals("https://127.0.0.1:443/gateway-path/topology1/cmf/home/", result);
    }
    ...
    public void testPrepareClusterExposedServices() {
        ...
        if (webHDFS.isPresent()) {
            assertEquals("https://10.0.0.1:443/gateway-path/topology2/webhdfs/v1", webHDFS.get().getServiceUrl());
        ...
        if (sparkHistoryUI.isPresent()) {
            assertEquals("https://10.0.0.1:443/gateway-path/topology2/sparkhistory/", sparkHistoryUI.get().getServiceUrl());
        ...
        if (hiveServer.isPresent()) {
            assertEquals("jdbc:hive2://10.0.0.1:443/;ssl=true;sslTrustStore=/cert/gateway.jks;trustStorePassword=${GATEWAY_JKS_PASSWORD};"
